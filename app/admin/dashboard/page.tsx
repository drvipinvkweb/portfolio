...
import { useLocalStorage, EventItem, SessionItem, BookingItem } from "@/lib/hooks";
import { Card, CardContent } from "@/components/ui/card";
import { Calendar, Search, Users } from "lucide-react";

export default function DashboardOverview() {
  const [events, , isClientE] = useLocalStorage<EventItem[]>("admin_upcoming_events", []);
  const [sessions, , isClientS] = useLocalStorage<SessionItem[]>("admin_past_sessions", []);
  const [bookings, , isClientB] = useLocalStorage<BookingItem[]>("admin_bookings", []);

  if (!isClientE || !isClientS || !isClientB) return null;
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 pt-4">
        ...
        <Card className="bg-[#1a1a1a] border-[#333]">
          <CardContent className="p-6 flex items-center justify-between">
            <div className="space-y-1">
              <p className="text-sm font-medium text-neutral-400">Past Sessions</p>
